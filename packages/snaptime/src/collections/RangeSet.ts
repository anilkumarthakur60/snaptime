...
export default class RangeSet implements Iterable<DateRange> {
  ...
  subtract(other: RangeSet | DateRange | [DateInput, DateInput]): RangeSet {
    const otherSet = other instanceof RangeSet ? other.merge() : new RangeSet([toDR(other)]).merge()
  ...
  gaps(lo?: DateInput, hi?: DateInput): RangeSet {
    ...
    if (lo != null && new DateTime(lo).isBefore(first.start)) {
      out.push(new DateRange(lo, first.start))
    }
    for (let i = 1; i < merged.length; i++) {
      out.push(new DateRange(merged[i - 1]!.end, merged[i]!.start))
    }
    if (hi != null && new DateTime(hi).isAfter(last.end)) {
      out.push(new DateRange(last.end, hi))
    }
    return new RangeSet(out)
  }
