...
export function adToBs(ad: Date | number): BSDate {
  ...
  const months = getMonths(bsYear)!
  // days < sum(months), so the loop always stops within the 12 entries
  let bsMonth = 1
...
export function bsToAd(year: number, month: number, day: number): Date {
  ...
  if (day < 1 || day > maxDay) {
    throw new RangeError(`BS day ${day} out of range for ${year}-${month} (max ${maxDay})`)
  }
...
export function formatBS(
  ...
  // bs.month is 1-12 and bs.weekday is 0-6 for any BSDate produced by adToBs
  const monthNe = BS_MONTH_NAMES_NE[bs.month - 1]!
  const monthEn = BS_MONTH_NAMES_EN[bs.month - 1]!
  const weekdayNe = BS_WEEKDAYS_NE[bs.weekday]!
  const weekdayEn = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][
    bs.weekday
  ]!
  const weekdayEnShort = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][bs.weekday]!
  ...
  const tokenMap: Record<string, string> = {
    ...
    DD: pad(bs.day),
    D: String(bs.day),
    dddd: weekdayLang === 'ne' ? weekdayNe : weekdayEn,
    ddd: weekdayLang === 'ne' ? weekdayNe.slice(0, -3) : weekdayEnShort,
    d: String(bs.weekday)
  }
