...
const BUILTIN_PATTERNS: Pattern[] = [
...

  {
    ...
    parse: (m, ref) => {
      // m[2] matched WEEKDAY_RE, so it is present and a key of WEEKDAYS
...

  {
    ...
    parse: (m, ref) => {
      // m[2] matched week|month|year, all keys of UNIT_MAP
...

  {
    ...
    parse: (m, ref) => {
      // m[1] matched MONTH_RE, so it is present and a key of MONTHS
