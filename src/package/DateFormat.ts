...
export class Duration {
  ...
  humanize(short = true): string {
    ...
    if (ms < 1000)
      return short ? `${Math.round(ms)}ms` : `${Math.round(ms)} milliseconds`;
    const s = ms / 1000;
...
export default class DateFormat {
  ...
  static parse(str = "", fmt = "", strict = false): DateFormat {
    ...
    if (strict) {
      if (parts.MM && (parts.MM < 1 || parts.MM > 12))
  ...
  diff(
    ...
    const o =
      other instanceof DateFormat
        ? other
        : new DateFormat(other as string | number | Date);
  ...
  isBefore(o: string | number | Date | DateFormat): boolean {
    return (
      this.valueOf() < new DateFormat(o as string | number | Date).valueOf()
    );
  }
  ...
  fromNow(): string {
    ...
    const str = new Duration(
      diff < 0
        ? -val * DateFormat.UNIT_MS[unit as Unit]
        : val * DateFormat.UNIT_MS[unit as Unit],
    ).humanize(false);
