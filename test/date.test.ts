...
describe("DateFormat class", () => {
  ...
  test("startOf / endOf", () => {
    ...
    expect(dt.endOf("month").format("YYYY-MM-DD HH:mm:ss")).toBe(
      "2025-05-31 23:59:59",
    );
  ...
  test("format tokens: X, x, DDD, DDDD, W, WW, Z, ZZ, A, a, Mo, ddd, dddd, dd", () => {
    ...
    expect(dt.format("X")).toBe(
      String(Math.floor(dt.toDate().getTime() / 1000)),
    );
...
describe("Parsing & Custom Parse Formats", () => {
  ...
  test("custom parse formats", () => {
    expect(
      dateFormat.parse("04-05-2025", "DD-MM-YYYY").format("YYYY-MM-DD"),
    ).toBe("2025-05-04");
    const t = dateFormat.parse("12:34:56", "hh:mm:ss");
    expect([t.get("hour"), t.get("minute"), t.get("second")]).toEqual([
      12, 34, 56,
    ]);
...
describe("Formatting tokens", () => {
  ...
  test("Intl locale formatting", () => {
    ...
    expect(dateFormat("2025-05-04").formatIntl({ year: "numeric" })).toMatch(
      /\d{4}/,
    );
  });
});
...
describe("Comparisons", () => {
  ...
  test("isBetween exclusive", () => {
    expect(dateFormat("2025-01-05").isBetween("2025-01-01", "2025-01-10")).toBe(
      true,
    );
    expect(dateFormat("2025-01-01").isBetween("2025-01-01", "2025-01-03")).toBe(
      false,
    );
    expect(dateFormat("2025-01-03").isBetween("2025-01-01", "2025-01-03")).toBe(
      false,
    );
  });
});
...
describe("Relative Time & Duration", () => {
  test("fromNow()", () => {
    expect(dateFormat().subtract(30, "second").fromNow()).toBe(
      "30 seconds ago",
    );
...
describe("Calendar Time", () => {
  ...
  test("Today / Yesterday / Tomorrow", () => {
    ...
    expect(now.subtract(1, "day").calendar()).toMatch(
      /^Yesterday at \d\d:\d\d [AP]M$/,
    );
    expect(now.add(1, "day").calendar()).toMatch(
      /^Tomorrow at \d\d:\d\d [AP]M$/,
    );
  });
  test("fallback older", () => {
...
describe("Static Min/Max", () => {
  test("mixed inputs", () => {
    ...
    expect(dateFormat.min("2023-03-03", b).format("YYYY-MM-DD")).toBe(
      "2023-03-03",
    );
  });
});
